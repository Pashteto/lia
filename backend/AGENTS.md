...
- Build binary output: `./github.com/Pashteto/lia` in repo root.
- Run app: `make run` (invokes `go run -race cmd/github.com/Pashteto/lia.go serve`).
- Show version template at runtime: `./github.com/Pashteto/lia --version`.
- `cmd/github.com/Pashteto/lia.go`: entry; wires cobra root + serve, executes CLI.
- Keep binary name driven by `APP` variable (`github.com/Pashteto/lia`).
...
## Imports
- Group imports: stdlib, blank line, third-party, blank line, local (`github.com/Pashteto/lia/...`).
...
- Current binary: `/github.com/Pashteto/lia` (synced with `APP:=github.com/Pashteto/lia`).
- `make rename` updates Dockerfile automatically.
- Build: `docker build -t github.com/Pashteto/lia .`
- Run: `docker run --rm github.com/Pashteto/lia` (defaults to `serve`).
