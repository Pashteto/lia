#!/usr/bin/env bash
# Inserts the 50 real venues (batch 2) from venues_seed_2.sql into the prod `venues` table
# on vds-ru215 (presence.tarski.ru). Idempotent (see venues_seed_2.sql header) —
...
SQL_FILE="$SCRIPT_DIR/venues_seed_2.sql"
BOX_HOST="vdska2"
BOX_DIR="/opt/lia/backend"
REMOTE_SQL_PATH="$BOX_DIR/db/seed/venues_seed_2.sql"
