...
- **Backend**: `docker compose --env-file .env.prod -f docker-compose.yml -f docker-compose.prod.yml up -d` in `/opt/lia/backend`. PostGIS (internal) + migrate + `backend-app` → `127.0.0.1:9080`. Creds in `.env.prod` (chmod 600, git-ignored). `HTTP_MOCK_AUTH=true` — known non-prod control.
