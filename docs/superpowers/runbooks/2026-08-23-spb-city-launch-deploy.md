...
координатами (venues 182). Обложек нет (плейсхолдеры) — follow-up.
...
- У 17 событий СПб нет обложек (в ленте плейсхолдеры) — можно догрузить
  изображения KudaGo (поле images) через upload.
