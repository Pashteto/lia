...
| `components/` | Composed pieces: `DiscoveryFeed`, `CreateEventForm`, `ComingSoon`, `ThemeToggle`. |
...
| Создание события | `design/screens/create-event.html` | ✅ built (`app/events/new`) — RHF + Zod, `POST /events` |
...
- Events carry `category` + `venue` (denormalized on the backend); cover images and organizer profiles are not in the model yet, so those degrade gracefully. Mock cover images come from Unsplash — see `next.config.ts` `images.remotePatterns`; swap for the real S3/CDN host later.
